...
void KD2DSurface::cdir()
{
    ...
    dx_assert(wic_factory_->CreateBitmap(100, 100,
					 ...
					 &wic_bitmap_));
    wic_converter_->GetSize(&kImageWidth, &kImageHeight);
...
void KD2DSurface::render()
{
    ...
    win_rt_->DrawText(kText, wcslen(kText), dwrite_text_format_, &kTextRect, win_text_brush_);
    win_rt_->DrawBitmap(d2d_bitmap_,
			D2D1::RectF(50.0,
				    10.0,
				    50.0 + (FLOAT)kImageWidth,
				    10.0 + (FLOAT)kImageHeight));
    d2d_bmp_->CopyFromMemory(&kBitmapDestRect, mem_, kBitmapPitch);
    win_rt_->DrawBitmap(d2d_bmp_,
			D2D1::RectF(50 + 130,
				    10,
				    50 + 130.0f + 100.0f,
				    10 + 100.0f));
